...
Add English pages under `src/content/docs`. Add Japanese translations with the same route under `src/content/docs/ja`. Add pages to the sidebar in `astro.config.mjs` when they should be part of the primary navigation.
...
Rust cells are fenced `rust` code blocks with metadata in leading `//|` or `///|` comments. The `id` must be unique within the page. English and Japanese versions may reuse the same local `id`; the build creates page-scoped internal IDs.
...
- `id`: required page-local cell identifier.
- `title`: heading shown in the cell UI.
- `run`: `button`, `reactive`, `autorun`, or `hidden`; defaults to `button`.
- `inputs`: named input controls made available as variables in the cell.
- `crates`: Rust helper crates; Rust only.
- `packages`: Pyodide packages; Python only.
- `timeoutMs`: positive timeout in milliseconds; defaults to `30000`.
- `showSource`: set to `false` to hide source by default.
