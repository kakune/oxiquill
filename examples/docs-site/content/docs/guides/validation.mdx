...
`pnpm wasm:dev` regenerates the runtime for local development. It requires `wasm32-wasi-ghc` when Haskell cells are present. `pnpm test:wasm` verifies generated Rust cell behavior. `pnpm test:e2e` verifies the built site in a browser, including Haskell cells.
...
- Missing Haskell compiler: install `wasm32-wasi-ghc` and rerun `pnpm wasm:dev` or `pnpm build`.
- Missing Haskell runtime: confirm `public/oxiquill/haskell-wasm` exists and rerun runtime generation.
