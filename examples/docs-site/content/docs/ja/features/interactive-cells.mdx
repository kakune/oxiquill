...
Haskell セルは standard library の code のみ対応します。`packages` や外部 helper module は使いません。
...
input name は cell 内の変数として使えます。Rust と Haskell セルでは安全な identifier に変換されます。記号だけが違う name は同じ binding に変換される可能性があるため避けます。
