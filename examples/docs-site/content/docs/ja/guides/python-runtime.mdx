...
download cache は Pyodide/runtime input ごとに versioned で、互換性のない release の file を再利用しません。local/CI build 間で保持すると network work を省けます。generated `.oxiquill` state と `public/oxiquill/pyodide` は verified cache から再生成できます。
