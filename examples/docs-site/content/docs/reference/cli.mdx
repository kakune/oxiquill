...
| `docgen`             | Synchronize the manifest; `--wasm dev` or `--wasm build` also builds required runtimes.    |
| `clean`              | Remove resolved generated/Astro output paths while preserving the verified download cache. |
