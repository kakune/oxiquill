...
Haskell cells use standard-library code only. They do not support `packages` or external helper modules.
...
Input names are available as variables inside the cell. In Rust and Haskell cells, names are converted into safe language identifiers. Avoid names that differ only by punctuation, because they may map to the same binding.
...
--|   label: { type: text, label: label, value: sample }
--|   includeSquares: { type: checkbox, label: include squares, value: true }
import Data.List (intercalate)

let base = [1 .. 4 :: Int]
let scaled = map (* factor) base
let displayed = if includeSquares then map (\value -> value * value) scaled else scaled
