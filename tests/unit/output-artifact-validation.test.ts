...
describe('output artifact validation', () => {
  ...
  it('rejects malformed artifact records, metadata, discriminators, and accessors', () => {
    ...
    expect(validationError(Object.assign(Object.create({ unsafe: true }), { kind: 'json', value: null }))).toContain(
      'custom prototype'
    );
    ...
    expect(validationError({ kind: 'chart', truncated: false, spec: { kind: 'line', series: [] } })).toContain(
      'does not support'
    );
    ...
    expect(validationError({ kind: 'text', stream: 'stdout', content: 'x', title: 1 })).toContain('when provided');
  });
  ...
  it('accepts exact text and HTML limits, truncates text, and rejects oversized HTML', () => {
    ...
    if (oversizedText.kind === 'text') {
      expect(new TextEncoder().encode(oversizedText.content).byteLength).toBeLessThanOrEqual(
  ...
  it('formats cyclic JSON and BigInt explicitly without invoking unsafe JSON operations', () => {
    ...
    expect(validationError({ kind: 'json', value: Object.assign(Object.create({}), { key: 'value' }) })).toContain(
      'custom prototype'
    );
  ...
  it('bounds deeply nested and oversized JSON while preserving a scoped result', () => {
    ...
    if (artifact.kind === 'json') {
      expect(new TextEncoder().encode(artifact.formattedValue).byteLength).toBeLessThanOrEqual(
        outputArtifactLimits.bytesPerTextJsonOrHtml
      );
    }
  });
  ...
  it('truncates tables at the row and column boundaries', () => {
    ...
    const columns = Array.from({ length: outputArtifactLimits.columnsPerTable + 1 }, (_, index) => ({
      key: String(index),
      label: String(index)
    }));
  ...
  it.each(['line', 'scatter', 'area'] as const)(
    ...
    (kind) => {
      const points = Array.from({ length: outputArtifactLimits.chartDataItems }, (_, index) => [index, index] as const);
      expect(validArtifact({ kind: 'chart', spec: { kind, series: [{ points }] } })).toMatchObject({ kind: 'chart' });
      expect(
        validationError({
          kind: 'chart',
  ...
  it('accepts the exact decoded image limit and rejects one byte over it', () => {
    ...
    expect(validArtifact({ kind: 'image', mime: 'image/png', data: exact.toString('base64') })).toMatchObject({
      kind: 'image'
    });
    ...
    expect(validationError({ kind: 'image', mime: 'image/png', data: oversized.toString('base64') })).toContain(
      'maximum'
    );
  });
  ...
  it('enforces per-run artifact and validated-byte limits without hiding valid siblings', () => {
    const artifactLimit = validateOutputArtifacts(
      Array.from({ length: outputArtifactLimits.artifactsPerRun + 1 }, () => ({
        kind: 'text',
        stream: 'stdout',
        content: 'x'
      }))
